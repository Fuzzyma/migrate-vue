...
```bash
npm install -D typescript@^6.0.0 vue-tsc@^3.3.8
```

Vite transpiles TypeScript but does not type-check it. Add a script such as `"type-check": "vue-tsc --noEmit"` and run it in CI. The version pin above avoids TypeScript 7 until `vue-tsc` supports its package exports.
...
<template>
  <div>
    <h1>Options Api</h1>
    <p>{{ msg }}</p>
    <button @click="hello">Log values</button>
  </div>
</template>
