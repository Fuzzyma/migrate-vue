...
<template>
  <button @click="emit('update:modelValue', props.modelValue + 1)">
    Click me ({{ props.modelValue }})
  </button>
  <button @click="emit('update:otherValue', props.otherValue + 1)">
    Click me ({{ props.otherValue }})
  </button>
</template>
...
Vue 2 used getters and setters and could not detect property addition after an object became reactive or direct array-index assignment. Those cases required `Vue.set()` / `this.$set()` or `splice()`. Reactive deletion used `Vue.delete()`, and changing an array's length also required `splice()`. Vue 3 uses proxies, so normal assignments and deletions are reactive when they are made through the proxied object.
...
Custom directives remain supported in Vue 3, but their lifecycle hooks now align with component lifecycle names. The main mappings are `bind` → `beforeMount`, `inserted` → `mounted`, `componentUpdated` → `updated`, and `unbind` → `unmounted`. The old `update` hook was removed; `created`, `beforeUpdate`, and `beforeUnmount` were added. Review every custom directive against the [official directive migration guide](https://v3-migration.vuejs.org/breaking-changes/custom-directives.html).
...
- BootstrapVue: The original library is Vue 2-first and supports Vue 3 only through global compat mode. For a native Vue 3 application, evaluate [BootstrapVueNext and its migration guide](https://bootstrap-vue-next.github.io/bootstrap-vue-next/docs/migration-guide.html).
