...
Aside from personal preference, the main reason not to switch is cost. Many libraries, such as [VueUse](https://vueuse.org/), expose composables that are most natural to use from the Composition Api. However, the Options Api remains an integral, fully supported part of Vue and there are no plans to deprecate it.
